...
if (APPLE AND WOLF_DB_FDB)
  get_filename_component(FDB_LIB_DIR ${FDB_C_LIBRARY} DIRECTORY)
  set_target_properties(wolf_tests PROPERTIES
    BUILD_RPATH "${FDB_LIB_DIR}"
    INSTALL_RPATH "${FDB_LIB_DIR}"
  )
endif()

target_include_directories(wolf_tests
  PRIVATE
    /usr/local/include
    ${Boost_INCLUDE_DIRS}
    ${CMAKE_CURRENT_SOURCE_DIR}
    ${CMAKE_CURRENT_SOURCE_DIR}/dep/proxygen/proxygen/_build/deps/include
    $<$<AND:$<BOOL:${WOLF_DB_FDB}>,$<PLATFORM_ID:Darwin>>:/usr/local/include/foundationdb>
    $<$<AND:$<BOOL:${WOLF_DB_FDB}>,$<PLATFORM_ID:Linux>>:/usr/include/foundationdb>
...
target_link_directories(wolf_tests
    PRIVATE
    /usr/local/lib/
    /opt/homebrew/lib/
...
target_link_libraries(wolf_tests
  PRIVATE
    ...
    gmock
    folly
    $<$<BOOL:${WOLF_DB_FDB}>:${FDB_C_LIBRARY}> 
    ${PROJECT_NAME}
)

if(UNIX AND NOT APPLE)
  target_link_libraries(wolf_tests PRIVATE unwind iberty)
endif()
